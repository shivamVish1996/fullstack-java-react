...
class Login extends Component {
			...
			.catch(error => {
				...
				if (error.response){
					if(error.response.status === 403) {
						this.setState({errorMessage: ErrorMessages.INVALID_USERNAME_OR_PASSWORD});
					} 
			  	} else {
	...
	render(){
		...
		if(this.state.errorMessage){
			alert = <Alert color="danger" className="CustomForm">{this.state.errorMessage}</Alert>
      	}
		...
		return (
			<div className="Login">
				...
				<br />
				{alert}
				<form onSubmit={this.submitLoginForm} className="CustomForm">
					...
					<button className="btn btn-outline-primary LoginBtn CustomForm"
					type="submit">Log in <i className="fa fa-sign-in fa-sm"></i></button>{" "}
					
					<button className="btn btn-outline-info CustomForm" type="reset" 
					onClick = {this.resetForm}>Reset <i className="fa fa-undo fa-sm"></i>
					</button>
					</div>
				</form>
